...
export default function BentoCard({
  ...
  const { type = 'image', imageUrl, videoUrl, text, title, bgColor, textColor, linkUrl,
          manualFontSize = null,
          mediaScale = 1, mediaOffsetX = 0, mediaOffsetY = 0 } = content
  ...
  useEffect(() => {
    if (isAdjusting) {
      ...
      setAdjOffsetX(mediaOffsetX)
      setAdjOffsetY(mediaOffsetY)
    }
  ...
  function handleAdjustSave() {
    dispatch({
      ...
      payload: { id, updates: { mediaScale: adjScale, mediaOffsetX: adjOffsetX, mediaOffsetY: adjOffsetY } },
    })
    onAdjustCancel?.()
  }
  ...
  return (
    ...
    >
      {/* Image background — inner div so we can apply transform for adjust mode */}
      {type === 'image' && imageUrl && (
        <div
          ...
          style={{
            ...
            transform: (activeScale !== 1 || activeOffsetX !== 0 || activeOffsetY !== 0)
              ? `scale(${activeScale}) translate(${activeOffsetX / activeScale}px, ${activeOffsetY / activeScale}px)`
              : undefined,
          }}
        />
      )}

      {/* Video background — wrapped so we can apply the same transform */}
      {type === 'video' && videoUrl && (
        <div
          className="absolute inset-0 pointer-events-none"
          style={{
